...
;; works only in R5RS. R6RS does not allow '_' as literal.
(define-syntax stream-match-pattern
  (syntax-rules (_)
    ((_ strm () (binding ...) body)
...
;;       (define (wildcard? x)
;;         (identifier? x)
;;         (identifier=? env x env '_))
;;       (let ((body (make-syntactic-closure env '() (cadddr (cdr form)))))
;;         (define (match-pattern strm pattern bindings)
;;           (cond ((null? pattern)
...
;;                    (lambda (env)
;;                      (let ((strm1 (make-syntactic-closure env '() 'strm)))
;;                        (if (wildcard? (car pattern))
;;                          `(and (stream-pair? ,strm)
;;                                (let ((,strm1 (stream-cdr ,strm)))
;;                                  ,(match-pattern
;;                                     strm1 (cdr pattern) bindings)))
;;                          (let ((temp (make-syntactic-closure env '() 'temp)))
;;                            `(and (stream-pair? ,strm)
;;                                  (let ((,temp (stream-car ,strm))
;;                                        (,strm1 (stream-cdr ,strm)))
;;                                    ,(match-pattern
;;                                       strm1 (cdr pattern)
;;                                       `((,(car pattern) ,temp)
;;                                         ,@bindings))))))))))
...
;;                         `(let ,bindings
;;                            ,body)
;;                         `(let ((,pattern ,strm) ,@bindings)
;;                            ,body)))))
;;         (match-pattern
;;           (make-syntactic-closure env '() (cadr form))
;;           (make-syntactic-closure env '() (caddr form))
;;           (make-syntactic-closure env '() (cadddr form)))))))
  ...
  (define stream-scan
    (stream-lambda (base strm)
      (if (stream-null? strm)
        (stream base)
        (stream-cons base
                     (stream-scan (proc base (stream-car strm))
                                  (stream-cdr strm))))))
  ...
  (define (result-stream->output-streams result-stream)
    (let loop ((i (len-values gen seed)) (outputs '()))
      (if (zero? i)
        ...
        (loop (- i 1)
              (cons (result-stream->output-stream result-stream i) outputs)))))
...
; vim: set lw+=stream-let,stream-match,stream-define,stream-lambda,syntax-rules:
